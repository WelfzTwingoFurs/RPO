...
#	$Label.text = str(blockX," ",blockY)
	
#	$Label.text = str(minY,maxY)
...
#		change_state(STATES.DOORSPAWN)
	...
	#goddammit this stayed broken for so long because I indented right and didn't notice
...
func _physics_process(delta):
	if faketimer > 0:
		if align == 1:
			...
			faketimer -= 0.5 #I think I broke this sometime
	elif faketimer < 1:
		if align == 1:
			faketimer = 0
		else:
	...
	if ouch == 0:
		if abs(motion.x) > 1 or abs(motion.y) > 1: # Walking animation #
			if faketimer < 30: # If almost ready to shoot
				...
				elif abs(type) == 2: #If a dangerous enemy, duck if the player is ducking
		...
		if align == 0:
			...
			if abs(playerX) < 45 && abs(playerY) < 15: ## beginning ##
				...
				if type == 0:
					...
					AniPlay.playback_speed = 0.25 #smart
				elif type == 1:
					AniPlay.playback_speed = 0.65 #ducker
